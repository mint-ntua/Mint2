...
<!--
  ...
  Provided for Linked Heritage project, 2011-11-18. 
...
<xsl:stylesheet version="2.0" 
	...
	<xsl:param name="provider">Judaica Europeana</xsl:param>
	...
	<xsl:template match="/">
		<europeana:metadata 
			...
			<xsl:variable name="europeana-output">
				<xsl:choose>
					<xsl:when test="lido:administrativeMetadata[1]/lido:recordWrap/lido:recordRights/lido:rightsType/lido:term[contains(., 'CC0 - mandatory only')]">mandatory-only</xsl:when>
					<xsl:when test="lido:administrativeMetadata[1]/lido:recordWrap/lido:recordRights/lido:rightsType/lido:term[contains(., 'CC0 - no descriptions')]">no-descriptions</xsl:when>
					<xsl:otherwise>full-transform</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
				...
				<xsl:when test="contains(lido:administrativeMetadata[1]/lido:recordWrap/lido:recordType, '/multipleResources')">
					...
					<xsl:for-each select="../../../lido:descriptiveMetadata">
						<xsl:choose>
							...
							<xsl:when test="$europeana-output='no-descriptions'">
								<xsl:call-template name="descriptiveMetadata-no-descriptions" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:call-template name="descriptiveMetadata-full-transform" />
							</xsl:otherwise>
						</xsl:choose>
					</xsl:for-each>
