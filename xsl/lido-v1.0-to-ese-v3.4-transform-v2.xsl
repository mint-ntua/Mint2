...
<!--
  ...
  Provided for Linked Heritage project, 2012-04-27. 

  version2: Updated according to LIDO v1.0 to ESE v3.4 mapping document.
					...
					<xsl:for-each select="lido:eventWrap/lido:eventSet/lido:event">
						...
						<xsl:for-each select="lido:eventDate">
							<xsl:choose>
								...
								<xsl:when test="not($acquisition and $europeana-output='no-descriptions') and lido:date">
									<xsl:for-each select="lido:date[string-length(lido:earliestDate)&gt;0]">
									<dc:date>
										<xsl:attribute name="xml:lang"><xsl:value-of select="$desclang" /></xsl:attribute>
										<xsl:choose>
								...
								<xsl:when test="not($acquisition and $europeana-output='no-descriptions') and lido:displayDate">
									<xsl:for-each select="lido:displayDate[string-length(.)&gt;0]">
									<dc:date>
								<xsl:call-template name="langattr">
									<xsl:with-param name="desclang" select="$desclang" />
	...
	<xsl:template name="work">
		...
		<xsl:choose>
			...
			<xsl:when test="lido:rightsWorkWrap/lido:rightsWorkSet/lido:rightsHolder/lido:legalBodyName">
				<xsl:for-each select="lido:rightsWorkWrap/lido:rightsWorkSet">
					<dc:rights>
					...
					<xsl:for-each select="lido:rightsHolder/lido:legalBodyName/lido:appellationValue[not(starts-with(@lido:pref, 'alternat'))]">
						<xsl:value-of select="."/>
