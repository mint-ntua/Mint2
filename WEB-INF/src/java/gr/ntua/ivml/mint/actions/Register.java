...
public class Register extends GeneralAction implements SessionAware{
    ...
    public String execute() throws Exception {
    	...
    	if(DB.getUserDAO().isEmailAvailable(getEmail())==false){
    		log.debug( "email in use" );
    		addActionError("Email address already registered");
    		return ERROR;
    	}
        ...
        String result="";
        if(isDefaultOrg()) {
        	  o=DB.getOrganizationDAO().findById(getDefaultOrg(), false);
          	/*check if there is a property for default org rights*/
          	if(Config.get("defaultOrgRights")!=null && Config.get("defaultOrgRights").length()>0){
          		 
          		 if(Config.get("emailAdminOnRegister")!=null && Config.getBoolean("emailAdminOnRegister")==true){ 
          			MailSender ms=new MailSender();
  	      	        String text="<BR>A user with login:<b>"+ user.getLogin()
  	      	                    +"</b> has registered for default oganization <b>"+ o.getName()+"</b>.<BR><BR> By default this user " +
  	      	                    "has viewing rights for that organization. If you would like to update his rights " +
  	      	                    "please use the administration page from the " + Config.get("mint.title") + " webtool.";
  	      	        String mail_to="";
  	      	        mail_to=Config.get("mail.admin");
  	      	        ...
  	      	        if(result.indexOf("Error")>-1){
  	      	        	addActionError("Email could not be sent to default organization's administrator to notify him for your registration. Please try again later.");
  	         		        return ERROR;}
  	      	        
          		 }
          		 user.setOrganization(o);
          		 user.setMintRole(Config.get("defaultOrgRights"));
          		 result="You have registered for the default organization. Currently you have only viewing rights for that organization. An email has been sent to the organization administrator to give you the appropriate rights.";
          	 }
          	else{
...
//    	        result="You have registered for the default test organization. Currently you have no rights for that organization. An email has been sent to the organization administrator to give you the appropriate rights.";
          ...
          /*check if we have to send email to mint_admin as well*/
